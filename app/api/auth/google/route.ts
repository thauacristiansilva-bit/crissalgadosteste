...
export async function POST(request: Request) {
  ...
  const key = authRateLimitKey("ip", `admin-google:${requestIp(request)}`)
  const state = checkAuthRateLimit(key, LIMIT, WINDOW_MS)
  if (!state.allowed) {
    return jsonError("Muitas tentativas. Tente novamente mais tarde.", 429, state.retryAfterSeconds)
  }

  const body = await request.json().catch(() => null) as { credential?: string } | null
  if (!body?.credential) {
    registerAuthFailure(key, WINDOW_MS)
    return jsonError("Não foi possível validar sua Conta Google.", 400)
  }

  try {
    const google = await verifyGoogleCredential(body.credential)
    const user = await authenticateAdminGoogleUser(google.subject, google.email)
    if (!user) {
      registerAuthFailure(key, WINDOW_MS)
      return jsonError(
    ...
    const tenantContext = await getDefaultAdminTenantContextForUserId(user.id)
    if (!tenantContext) {
      registerAuthFailure(key, WINDOW_MS)
      return jsonError("Não foi possível entrar nesta conta.", 403)
    }

    const response = NextResponse.json({ ok: true, authSource: "google" })
    response.headers.set("Cache-Control", "no-store")
    ...
    return jsonError("Não foi possível entrar com Google no momento.", 503)
  }
}
