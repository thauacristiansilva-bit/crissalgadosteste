...
$baseCommit = "87279a5"
$tempNode = Join-Path $env:TEMP "saborflow-fix-order-sound-v2.cjs"

if (-not (Test-Path $arquivo)) {
  throw "Arquivo nao encontrado: $arquivo"
}

Write-Host "1/4 Restaurando o painel correto do commit $baseCommit..."
git restore --source=$baseCommit --worktree -- "$arquivo"
if ($LASTEXITCODE -ne 0) {
  throw "Falha ao restaurar o arquivo."
}
...
// Normaliza CRLF/LF para evitar falha de correspondencia no Windows.
...
console.log("Arquivo salvo em UTF-8.");
'@
...
Write-Host "2/4 Reaplicando o alerta sonoro em UTF-8..."
node $tempNode
if ($LASTEXITCODE -ne 0) {
...
Write-Host "3/4 Verificando os textos principais..."
...
foreach ($esperado in $esperados) {
  if (-not $texto.Contains($esperado)) {
    throw "Texto UTF-8 esperado nao encontrado: $esperado"
  }
}

Write-Host "4/4 Verificando diff..."
...
Write-Host ""
Write-Host "CORRECAO OK."
Write-Host "- acentos restaurados"
Write-Host "- alerta Som ON/OFF mantido"
Write-Host "- arquivo em UTF-8"
