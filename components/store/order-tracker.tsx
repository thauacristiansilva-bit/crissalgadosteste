        ...
        <section className="overflow-hidden rounded-3xl border border-black/5 bg-white shadow-xl">
          ...
          <div className="p-5 sm:p-8">
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-2">
              <section className="rounded-2xl border border-gray-200 p-4"><h2 className="font-black">Itens</h2><div className="mt-3 space-y-2">{order.items.map((item, index) => <div key={`${item.productId}-${index}`} className="flex items-start justify-between gap-3 text-sm"><div><span><strong>{item.quantity}x</strong> {item.name}</span>{item.modifiers?.length ? <div className="mt-1 space-y-0.5 pl-4">{item.modifiers.map((modifier, modifierIndex) => <p key={`${modifier.groupId}-${modifier.optionId}-${modifierIndex}`} className="text-xs text-gray-500">+ {modifier.optionName}{modifier.included ? " · incluído" : modifier.priceDelta > 0 ? ` · + ${money(modifier.priceDelta)}` : ""}</p>)}</div> : null}</div><strong>{money(item.subtotal)}</strong></div>)}</div><div className="mt-3 border-t border-gray-100 pt-3"><div className="flex justify-between text-xs text-gray-500"><span>Subtotal</span><span>{money(order.subtotal)}</span></div>{order.discount > 0 && <div className="mt-1 flex justify-between text-xs font-bold text-emerald-700"><span>Desconto {order.couponCode ? `(${order.couponCode})` : ""}</span><span>-{money(order.discount)}</span></div>}{order.deliveryFee > 0 && <div className="mt-1 flex justify-between text-xs text-gray-500"><span>Entrega</span><span>{money(order.deliveryFee)}</span></div>}<div className="mt-2 flex justify-between"><strong>Total</strong><strong className="text-lg">{money(order.total)}</strong></div></div></section>
