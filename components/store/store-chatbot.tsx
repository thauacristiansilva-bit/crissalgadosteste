...
export function StoreChatbot({ settings }: { settings: StoreSettings }) {
  ...
  return <div className="fixed bottom-5 right-5 z-[90]">
    {open && <div className="mb-3 w-[min(340px,calc(100vw-32px))] overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-2xl">
      <div className="flex items-center justify-between bg-slate-950 px-4 py-3 text-white"><div className="flex items-center gap-2"><Bot className="h-5 w-5" /><strong>Atendimento</strong></div><button onClick={() => setOpen(false)}><X className="h-4 w-4" /></button></div>
    ...
    <button onClick={() => setOpen((value) => !value)} className="ml-auto flex h-14 w-14 items-center justify-center rounded-full bg-slate-950 text-white shadow-xl" aria-label="Abrir atendimento"><Bot className="h-6 w-6" /></button>
  </div>
}
