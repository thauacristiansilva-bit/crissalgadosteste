...
export function ChatbotPanel({ settings, onSettingsChanged }: { settings: StoreSettings; onSettingsChanged: (settings: StoreSettings) => void }) {
  ...
  async function save() {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error || "Erro ao salvar chatbot.")
      onSettingsChanged(data.settings)
      setMessage("Chatbot atualizado.")
    } catch (error) {
  ...
  return <div className="grid gap-5 xl:grid-cols-[1fr_.7fr]">
    <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
      <div className="flex items-center gap-3"><div className="rounded-xl bg-blue-50 p-2.5 text-blue-700"><Bot className="h-5 w-5" /></div><div><h2 className="text-lg font-black">Chatbot do cardápio</h2><p className="text-sm text-gray-500">Atendimento rápido no próprio site, com atalhos para dúvidas comuns.</p></div></div>
      <label className="mt-5 flex items-center justify-between rounded-xl border border-gray-200 p-4"><span><strong className="block text-sm">Ativar chatbot</strong><small className="text-gray-500">Exibe o botão de ajuda no cardápio do cliente.</small></span><input type="checkbox" checked={enabled} onChange={(e) => setEnabled(e.target.checked)} className="h-5 w-5" /></label>
      ...
      <button onClick={save} disabled={busy} className="mt-4 inline-flex h-11 items-center gap-2 rounded-xl bg-blue-700 px-4 text-sm font-black text-white disabled:opacity-50"><Save className="h-4 w-4" />{busy ? "Salvando..." : "Salvar chatbot"}</button>
      {message && <p className="mt-3 rounded-xl bg-blue-50 px-3 py-2 text-xs font-bold text-blue-700">{message}</p>}
    </section>
    <aside className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
      <h2 className="font-black">Respostas disponíveis</h2>
      ...
      <p className="mt-4 text-xs text-gray-400">Este chatbot é local e não envia dados para IA. Uma integração futura com WhatsApp Business Cloud API pode automatizar atendimento externo.</p>
    </aside>
  </div>
}
