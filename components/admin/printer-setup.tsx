...
export function PrinterSetup({ selectedAgentId, selectedPrinterName, onSelected }: {
  ...
  async function create() {
    ...
    try {
      ...
      await refresh()
      setMessage("Conexão criada. Baixe o arquivo neste computador da impressora.")
  ...
  async function download() {
    ...
    setBusy(true); setMessage("")
    try {
      const response = await fetch("/api/admin/print-agent-installer", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ token }) })
  ...
  return <div className="mt-4 space-y-3 rounded-2xl border border-blue-100 bg-blue-50 p-4 text-sm text-blue-950">
    ...
    <p>1. Instale ou conecte a impressora no Windows deste computador. 2. Baixe e execute o conector uma vez. 3. Escolha aqui a impressora que aparecer.</p>
    <div className="flex flex-wrap gap-2"><input value={name} onChange={event => setName(event.target.value)} placeholder="Nome deste computador" className="min-w-[180px] flex-1 rounded-xl border bg-white px-3 py-2" /><button type="button" disabled={busy || !name.trim()} onClick={() => void create()} className="rounded-xl bg-blue-700 px-4 py-2 font-bold text-white disabled:opacity-50">Preparar computador</button>{token && <button type="button" disabled={busy} onClick={() => void download()} className="rounded-xl border border-blue-300 bg-white px-4 py-2 font-bold">Baixar conector</button>}</div>
