              ...
              <div className="mt-4 space-y-4">
                {groups.map((group, groupIndex) => <article key={group.key} className="rounded-2xl border border-gray-200 bg-gray-50 p-4">
                  <div className="grid gap-3 lg:grid-cols-[1.2fr_1.2fr_110px_110px_120px_auto]">
                    ...
                    <label className="text-[11px] font-bold text-gray-500"><HelpLabel helpKey="composition.min">Mínimo</HelpLabel><input type="number" min="0" value={group.minSelect} onChange={(event) => setGroups((current) => current.map((item) => item.key === group.key ? { ...item, minSelect: event.target.value } : item))} className="mt-1 h-9 w-full rounded-lg border border-gray-200 bg-white px-2 text-sm"/></label>
                    <label className="text-[11px] font-bold text-gray-500"><HelpLabel helpKey="composition.max">Máximo</HelpLabel><input type="number" min="1" value={group.maxSelect} onChange={(event) => setGroups((current) => current.map((item) => item.key === group.key ? { ...item, maxSelect: event.target.value } : item))} className="mt-1 h-9 w-full rounded-lg border border-gray-200 bg-white px-2 text-sm"/></label>
                    <label className="text-[11px] font-bold text-gray-500"><HelpLabel helpKey="composition.included">Incluídos grátis</HelpLabel><input type="number" min="0" value={group.includedQuantity} onChange={(event) => setGroups((current) => current.map((item) => item.key === group.key ? { ...item, includedQuantity: event.target.value } : item))} className="mt-1 h-9 w-full rounded-lg border border-gray-200 bg-white px-2 text-sm"/></label>
                  ...
                  <div className="mt-4 space-y-3">
                    {group.options.map((option, optionIndex) => <div key={option.key} className="rounded-xl border border-gray-200 bg-white p-3">
                      <div className="grid gap-2 sm:grid-cols-[1fr_1fr_130px_auto]">
                        ...
                        <label className="relative"><span className="absolute right-2 top-1/2 z-10 -translate-y-1/2"><HelpTip helpKey="composition.optionPrice" /></span><span className="absolute left-2 top-1/2 -translate-y-1/2 text-xs font-bold text-gray-400">R$</span><input value={option.priceDelta} onChange={(event) => setGroups((current) => current.map((item) => item.key !== group.key ? item : { ...item, options: item.options.map((candidate) => candidate.key === option.key ? { ...candidate, priceDelta: event.target.value } : candidate) }))} inputMode="decimal" className="h-9 w-full rounded-lg border border-gray-200 pl-8 pr-8 text-sm"/></label>
