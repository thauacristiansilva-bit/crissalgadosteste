...
import { Bot, FileText, MessageCircle, MonitorSmartphone, Printer } from "lucide-react"
...
import { MetaSignupButton } from "@/components/admin/meta-signup-button"
...
export function SimpleConnectionsPanel({ settings, organizationSlug, onSettingsChanged }: {
  ...
  return <div className="mx-auto max-w-5xl space-y-5">
    ...
    <section className="rounded-3xl border bg-white p-5 shadow-sm"><h3 className="flex items-center gap-2 text-lg font-black"><MessageCircle className="h-5 w-5" />WhatsApp e atendimento</h3>
      ...
      {!configured && <MetaSignupButton onConnected={() => void load()} />}
      ...
      {configured && <div className="mt-4 grid gap-3 sm:grid-cols-2">
        <button type="button" disabled={busy || !workerReady || (!whatsapp!.settings.templateName && !whatsapp!.settings.orderNotificationsEnabled)} onClick={() => void setOrderAlerts(!whatsapp!.settings.orderNotificationsEnabled)} className="rounded-xl border p-4 text-left disabled:opacity-50"><strong className="block text-sm">Aviso ao finalizar pedido</strong><span className="text-xs text-slate-600">{whatsapp!.settings.orderNotificationsEnabled ? "Ativado — toque para pausar" : whatsapp!.settings.templateName ? "Desativado — toque para ativar" : "Aguardando modelo aprovado na Meta"}</span></button>
    ...
    <section className="rounded-3xl border bg-white p-5 shadow-sm"><h3 className="flex items-center gap-2 text-lg font-black"><Printer className="h-5 w-5" />Impressora</h3><p className="mt-2 text-sm text-slate-600">Encontre no painel a impressora instalada no Windows, Mac ou Linux da loja.</p><div className="mt-3 grid gap-3 sm:grid-cols-2"><label className="text-sm font-bold">Modelo (opcional)<input className="mt-1 w-full rounded-xl border px-3 py-2 font-normal" placeholder="Ex.: Elgin i9" value={settings.printerModel || ""} onChange={event => onSettingsChanged({ ...settings, printerModel: event.target.value })} onBlur={() => void saveSettings({ printerModel: settings.printerModel || "" }, "Modelo da impressora salvo.")} /></label><label className="text-sm font-bold">Largura do papel<select value={settings.printerPaperWidthMm || 80} onChange={event => void saveSettings({ printerPaperWidthMm: Number(event.target.value) as 50 | 58 | 80 }, "Largura do papel salva para a impressão automática.")} className="mt-1 w-full rounded-xl border bg-white px-3 py-2"><option value={50}>50 mm</option><option value={58}>58 mm</option><option value={80}>80 mm</option></select></label></div><p className="mt-2 text-xs text-slate-500">Escolha a largura real do rolo. O modelo é apenas para identificação; o driver continua instalado no computador.</p><PrinterSetup selectedAgentId={settings.printerAgentId} selectedPrinterName={settings.printerName} onSelected={(agentId, printerName) => void saveSettings({ printerAgentId: agentId, printerName, autoPrintNewOrders: true }, `Impressora ${printerName} selecionada.`)} /></section>
