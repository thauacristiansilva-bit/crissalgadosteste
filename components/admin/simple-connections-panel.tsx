...
export function SimpleConnectionsPanel({ settings, organizationSlug, onSettingsChanged }: {
  ...
  return <div className="mx-auto max-w-5xl space-y-5">
    ...
    <section className="rounded-3xl border bg-white p-5 shadow-sm"><h3 className="flex items-center gap-2 text-lg font-black"><Printer className="h-5 w-5" />Impressora</h3><p className="mt-2 text-sm text-slate-600">Encontre no painel a impressora USB ou Wi‑Fi instalada no computador da loja.</p><PrinterSetup selectedAgentId={settings.printerAgentId} selectedPrinterName={settings.printerName} onSelected={(agentId, printerName) => void saveSettings({ printerAgentId: agentId, printerName, autoPrintNewOrders: true }, `Impressora ${printerName} selecionada.`)} /></section>
