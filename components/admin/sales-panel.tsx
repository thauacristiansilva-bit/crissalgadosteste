...
export function SalesPanel({ orders, settings, initialCashSessions, initialEntries }: { orders: Order[]; settings: StoreSettings; initialCashSessions: CashSession[]; initialEntries: FinancialEntry[] }) {
  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-5 xl:grid-cols-2">
      <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm"><div className="flex items-center justify-between"><div><div className="flex items-center gap-1.5"><h2 className="font-black">Caixa</h2><HelpTip helpKey="cash.openClose" /></div><p className="text-sm text-gray-500">Abertura e fechamento do turno.</p></div><span className={`rounded-full px-3 py-1 text-xs font-black ${openCash ? "bg-emerald-100 text-emerald-700" : "bg-gray-100 text-gray-500"}`}>{openCash ? "ABERTO" : "FECHADO"}</span></div><div className="mt-4 flex gap-2"><input type="number" step="0.01" value={cashAmount} onChange={(e) => setCashAmount(e.target.value)} className="h-11 min-w-0 flex-1 rounded-xl border border-gray-200 px-3 text-sm" placeholder={openCash ? "Valor contado no fechamento" : "Troco inicial"}/><button onClick={() => cashAction(openCash ? "close" : "open")} className={`inline-flex h-11 items-center gap-2 rounded-xl px-4 text-sm font-black text-white ${openCash ? "bg-red-600" : "bg-blue-700"}`}>{openCash ? <LockKeyhole className="h-4 w-4"/> : <UnlockKeyhole className="h-4 w-4"/>}{openCash ? "Fechar caixa" : "Abrir caixa"}</button></div>{openCash && <p className="mt-3 text-xs text-gray-500">Aberto em {date(openCash.openedAt)} · fundo {money(openCash.openingAmount)}</p>}{message && <p className="mt-3 rounded-xl bg-blue-50 px-3 py-2 text-xs font-bold text-blue-700">{message}</p>}</section>

      <form onSubmit={addEntry} className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm"><div className="flex items-center gap-1.5"><h2 className="font-black">Lançamento financeiro</h2><HelpTip helpKey="finance.entry" /></div><p className="text-sm text-gray-500">Registre apenas receitas fora dos pedidos e despesas da operação.</p><div className="mt-3 rounded-xl bg-amber-50 px-3 py-2 text-xs font-bold text-amber-800">As vendas dos pedidos já entram automaticamente na DRE. Não lance a mesma venda aqui para evitar receita duplicada.</div><div className="mt-4 grid gap-3 sm:grid-cols-2"><select value={entryType} onChange={(e) => setEntryType(e.target.value as any)} className="h-10 rounded-xl border border-gray-200 bg-white px-3 text-sm"><option value="expense">Despesa</option><option value="income">Outra receita</option></select><div className="relative"><div className="absolute right-2 top-1/2 z-10 -translate-y-1/2"><HelpTip helpKey="finance.category" /></div><input list="dre-financial-categories" value={entryCategory} onChange={(e) => setEntryCategory(e.target.value)} className="h-10 w-full rounded-xl border border-gray-200 px-3 pr-9 text-sm" placeholder="Categoria da DRE"/><datalist id="dre-financial-categories">{dreCategories.map((category) => <option key={category} value={category}/>)}</datalist></div><input required value={entryDescription} onChange={(e) => setEntryDescription(e.target.value)} className="h-10 rounded-xl border border-gray-200 px-3 text-sm sm:col-span-2" placeholder="Descrição"/><input required value={entryAmount} onChange={(e) => setEntryAmount(e.target.value)} inputMode="decimal" className="h-10 rounded-xl border border-gray-200 px-3 text-sm" placeholder="Valor"/><button className="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-gray-950 px-4 text-sm font-black text-white"><Plus className="h-4 w-4"/>Salvar</button></div></form>
    </div>
