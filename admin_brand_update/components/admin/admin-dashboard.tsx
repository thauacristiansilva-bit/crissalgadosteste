...
export function AdminDashboard({ initialData, adminEmail }: { initialData: DashboardData; adminEmail: string }) {
  ...
  const nav = (
    <>
      <div className="border-b px-5 py-5" style={{ borderColor: "rgba(255,255,255,0.12)" }}>
        <div className="rounded-2xl border p-4 shadow-sm" style={{ background: "linear-gradient(135deg, rgba(255,255,255,0.08), rgba(255,255,255,0.02))", borderColor: "rgba(255,255,255,0.12)" }}>
          <div className="mb-3 flex items-center gap-3">
            <img src="/saborflow-brand.png" alt="SaborFlow" className="h-12 w-auto shrink-0 object-contain" />
            <div className="min-w-0">
              <p className="text-xs font-black uppercase tracking-[0.25em]" style={{ color: "#ffd7ad" }}>Marca registrada</p>
              <p className="text-sm font-black text-white">SaborFlow®</p>
              <p className="text-[11px] leading-4" style={{ color: "#f5c996" }}>Orders • Kitchen • Delivery • Insights</p>
            </div>
          </div>
          <div className="flex items-center gap-3 rounded-2xl border px-3 py-3" style={{ backgroundColor: "rgba(255,255,255,0.06)", borderColor: "rgba(255,255,255,0.08)" }}>
            <div className="flex h-12 w-12 items-center justify-center overflow-hidden rounded-2xl border bg-white" style={{ borderColor: "rgba(255,255,255,0.15)" }}>
              {settings.logoImage ? <img src={settings.logoImage} alt="Logo da loja" className="h-full w-full object-cover" /> : <Store className="h-5 w-5" style={{ color: brand.orangeDark }} />}
            </div>
            <div className="min-w-0">
              <p className="truncate text-sm font-black text-white">{settings.storeName}</p>
              <p className="truncate text-[11px]" style={{ color: "#f7d9b7" }}>{settings.systemName || "SaborFlow"} · Painel Admin</p>
            </div>
          </div>
        </div>
      </div>
      <nav className="flex-1 space-y-1 overflow-y-auto px-3 py-5">
        <p className="px-3 pb-2 text-[10px] font-bold uppercase tracking-[0.2em]" style={{ color: "#ffd7ad" }}>Operação</p>
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              type="button"
              className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm font-semibold transition ${active ? "shadow-sm" : "hover:bg-white/8"}`}
              style={active ? { backgroundColor: brand.cream, color: brand.brown } : { color: "#fff5ea" }}
            >
              <Icon className="h-[18px] w-[18px]" style={active ? { color: brand.orangeDark } : { color: "#ffd7ad" }} />
              {item.label}
              {item.key === "orders" && <span className="ml-auto rounded-full px-2 py-0.5 text-[10px] font-bold" style={active ? { backgroundColor: brand.orange, color: "white" } : { backgroundColor: "rgba(255,255,255,0.12)", color: "white" }}>{summary.openOrders + summary.readyOrders}</span>}
              {item.key === "inventory" && products.some((p) => p.trackStock && p.stock <= p.minStock) && <span className="ml-auto h-2 w-2 rounded-full" style={{ backgroundColor: "#fbbf24" }} />}
            </button>
          )
        })}
      </nav>
      <div className="border-t p-4" style={{ borderColor: "rgba(255,255,255,0.12)" }}>
        <a href="/" target="_blank" className="mb-3 flex w-full items-center justify-center gap-2 rounded-xl px-3 py-2 text-xs font-bold text-white" style={{ background: `linear-gradient(135deg, ${brand.orange}, ${brand.orangeDark})` }}><Store className="h-4 w-4" />Abrir site do cliente</a>
        <div className="mb-3 flex items-center gap-3 rounded-xl p-3" style={{ backgroundColor: "rgba(255,255,255,0.06)" }}><div className="flex h-9 w-9 items-center justify-center rounded-full text-xs font-black text-white" style={{ background: `linear-gradient(135deg, ${brand.orange}, ${brand.orangeDark})` }}>{(settings.storeName || "SA").slice(0,2).toUpperCase()}</div><div className="min-w-0 flex-1"><p className="text-xs font-bold text-white">Administrador</p><p className="truncate text-[11px]" style={{ color: "#f7d9b7" }}>{adminEmail}</p></div></div>
        <button onClick={logout} disabled={loggingOut} type="button" className="flex w-full items-center justify-center gap-2 rounded-xl border px-3 py-2 text-xs font-bold text-white transition disabled:opacity-50" style={{ borderColor: "rgba(255,255,255,0.14)" }}><LogOut className="h-4 w-4" />{loggingOut ? "Saindo..." : "Sair"}</button>
      </div>
    </>
  )
    ...
    <div className="min-h-screen text-gray-950 lg:flex" style={{ backgroundColor: brand.cream }}>
      ...
      <div className="min-w-0 flex-1">
        <header className="sticky top-0 z-30 flex h-20 items-center justify-between border-b bg-white/95 px-4 backdrop-blur sm:px-6" style={{ borderColor: brand.border }}>
          <div className="flex items-center gap-3"><button onClick={() => setMobileNav(true)} type="button" className="rounded-xl border p-2 text-gray-700 lg:hidden" style={{ borderColor: brand.border }} aria-label="Abrir menu"><Menu className="h-5 w-5" /></button><div className="hidden md:flex items-center gap-3 rounded-2xl border bg-white px-3 py-2 shadow-sm" style={{ borderColor: brand.border }}><img src="/saborflow-brand.png" alt="SaborFlow" className="h-10 w-auto object-contain" /><div className="leading-tight"><p className="text-[11px] font-black uppercase tracking-[0.2em]" style={{ color: brand.orangeDark }}>Marca registrada</p><p className="text-sm font-black" style={{ color: brand.brown }}>SaborFlow®</p></div></div><div><h1 className="font-black text-gray-950">{title}</h1><p className="hidden text-xs text-gray-500 sm:block">{settings.storeName} · {settings.city} - {settings.state}</p></div></div>
          <div className="flex items-center gap-2"><span className={`hidden rounded-full px-3 py-1.5 text-xs font-bold sm:inline-flex ${operatingNow ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-700"}`}>● {operatingNow ? "Loja aberta" : settings.acceptingOrders ? "Fora do expediente" : "Pedidos pausados"}</span><button onClick={() => changeSection("orders")} type="button" className="relative rounded-xl border p-2.5 text-gray-600 hover:bg-gray-50" style={{ borderColor: brand.border }} aria-label="Notificações"><Bell className="h-4 w-4" />{summary.openOrders > 0 && <span className="absolute -right-1 -top-1 flex h-5 min-w-5 items-center justify-center rounded-full px-1 text-[10px] font-bold text-white" style={{ backgroundColor: brand.orangeDark }}>{summary.openOrders}</span>}</button></div>
        </header>

        <main className="mx-auto max-w-[1600px] p-4 sm:p-6">
          <div className="mb-5 flex items-center gap-3 rounded-2xl border bg-white px-4 py-3 shadow-sm" style={{ borderColor: brand.border }}>
            <img src="/saborflow-brand.png" alt="SaborFlow" className="h-11 w-auto object-contain" />
            <div className="min-w-0">
              <p className="text-[11px] font-black uppercase tracking-[0.25em]" style={{ color: brand.orangeDark }}>Marca registrada do sistema</p>
              <p className="text-sm font-bold" style={{ color: brand.brown }}>SaborFlow® permanece fixo como assinatura da plataforma.</p>
            </div>
          </div>
